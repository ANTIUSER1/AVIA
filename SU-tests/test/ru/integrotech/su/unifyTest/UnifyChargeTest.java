...
/**
 * Unify test for ru.aeroflot.fmc.io.charge Compares response of Charge with
 * pre-saved response Has possibilities of creation test for any requests. To
 * create new tests, follow next steps 1. Create root path for ChargeRoute unify
 * tests in your local machine 2. Define path to this directory in
 * test.ru.aeroflot.fmc.resources.unifyTest.properties in uniChargeTestDirectory
 * property, for example " uniChargeTestDirectory=D:/fpm_tests/charge/ " 3.
 * Create test case directory inside root directory, for example "SVO-LED-00" 4.
 * Put file "request.json" inside SVO-LED-00 directory 5. Put file
 * "expectedResponse.json" inside SVO-LED-00 directory 6. Create another test
 * directory, for example "LED-UFA-00" and repeat steps 4 and 5 for this new
...
public class UnifyChargeTest extends UnifyBaseTest {
	...
	protected boolean isCorrectCase(String pathToCaseFolder) throws IOException {
		try {
			...
			JsonElement jsonElement = loader.loadJson(pathToCaseFolder
					+ REQUEST_FILE_NAME);
			ChargeInput chargeInput = loader.getTestsCache()
					.loadChargeInputParams(jsonElement);
			List<ChargeRoute> actualChargeRoutes = this.chargeBuilder
					.getChargeRoutes(chargeInput);

			jsonElement = loader.loadJson(pathToCaseFolder
					+ EXPECTED_RESPONSE_FILE_NAME);
			List<ChargeRoute> expectedChargeRoutes = loader.getTestsCache()
					.loadChargeRoutes(jsonElement);

			String testHeader = this.buildReportHeader(chargeInput);
			String testBody = this.compareChargeRoute(expectedChargeRoutes,
					actualChargeRoutes);
	...
	protected String buildReportHeader(ChargeInput chargeInput) {
		...
		builder.append("|------------------------------------|\n");
		builder.append(String.format("|  %-20s %-10s   |\n",
				"tier level code:", chargeInput.getTierLevel()
						.getTierLevelCode()));
		builder.append(String.format("|  %-20s %-10s   |\n", "is round trip:",
				chargeInput.getIsRoundTrip()));
		builder.append(" ------------------------------------\n");
		builder.append("\n");
	...
	protected String compareChargeRoute(List<ChargeRoute> expected,
			List<ChargeRoute> actual) {
		StringBuilder builder = new StringBuilder();
