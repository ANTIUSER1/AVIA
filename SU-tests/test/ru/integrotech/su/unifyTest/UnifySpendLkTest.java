...
/**
 * Unify test for ru.aeroflot.fmc.io.output.spend.SpendLkRoute Compares response
 * of SpendLK with pre-saved response Has possibilities of creation test for any
 * requests. To create new tests, follow next steps 1. Create root path for
 * spendLk unify tests in your local machine 2. Define path to this directory in
 * test.ru.aeroflot.fmc.resources.unifyTest.properties in
 * uniSpendLkTestDirectory property, for example
 * " uniSpendTestDirectory=D:/fpm_tests/spend_lk/ " 3. Create test case
 * directory inside root directory, for example "SVO-JFK-00" 4. Put file
 * "request.json" inside SVO-JFK-00 directory 5. Put file
 * "expectedResponse.json" inside SVO-JFK-00 directory 6. Create another test
 * directory, for example "SVO-NRT-01" and repeat steps 4 and 5 for this new
 * directory. 7. Run public void test(). Program test each folder, test is OK if
 * actual response is equals to your expectedResponse.json Program will say OK
 * if test is ok or write right answer in exact directory as
 * "actualResponse.json".
 */
...
public class UnifySpendLkTest extends UnifyBaseTest {
	...
	protected boolean isCorrectCase(String pathToCaseFolder) throws IOException {
		try {
			...
			List<SpendLkRoute> actualSpendLkRoutes = this.spendLkBuilder
					.getSpendLkRoutes(spendInput);

			jsonElement = loader.loadJson(pathToCaseFolder
					+ EXPECTED_RESPONSE_FILE_NAME);
			List<SpendLkRoute> expectedSpendLkRoutes = loader.getTestsCache()
					.loadSpendLkRoutes(jsonElement);

			String testHeader = this.buildReportHeader(spendInput);
			String testBody = this.compareSpendRoute(expectedSpendLkRoutes,
					actualSpendLkRoutes);
	...
	protected String compareSpendRoute(List<SpendLkRoute> expected,
			List<SpendLkRoute> actual) {
		StringBuilder builder = new StringBuilder();
	...
	private String getRoteCode(SpendLkRoute route) {
		String builder = String.format("%s  ", route.getOrigin().getAirport()
				.getAirportCode())
				+ String.format("%s ", route.getDestination().getAirport()
						.getAirportCode());
		return builder;
	}
	...
	private boolean isDeepEquals(SpendLkRoute expected, SpendLkRoute actual) {
		return Objects.equals(expected.getRequiredAwards(),
				actual.getRequiredAwards());
	}
