...
public class RoutesToString {
	...
	public void init() {
		this.routesBuilder = RoutesBuilder.of(MockLoader.getInstance()
				.getRegisterCache());
		this.bonusSearcher = BonusSearcher.of(MockLoader.getInstance()
				.getRegisterCache());
	}
	...
	public void printRoutesWithDetails() throws Exception {
		...
		for (Route route : routes) {
			...
			if (route.getAflBonuses() != null
					&& route.getAflBonuses().size() > 0) {
				System.out.printf("\n  AFL bonuses: %d", route.getAflBonuses()
						.size());
			...
			if (route.getScyteamBonuses() != null
				...
				System.out.printf("\n  Skyteam bonuses: %d", route
						.getScyteamBonuses().size());
